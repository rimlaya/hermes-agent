...
class TestReplyToText:
    ...
    @pytest.mark.asyncio
    async def test_reference_without_resolved_fetches_original_message(self, reply_text_adapter):
        """Unresolved Discord references are fetched so reply chains keep full text."""
        ref = SimpleNamespace(message_id=555, resolved=None)
        channel = FakeDMChannel()
        channel.fetch_message = AsyncMock(return_value=SimpleNamespace(content="fetched original text"))
