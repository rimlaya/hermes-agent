...
def test_discord_trusted_agent_bot_authorized_when_allow_bots_unset(monkeypatch):
    """Configured private-agent guidance bots bypass the generic bot deny policy."""
    runner = _make_bare_runner()

    monkeypatch.delenv("DISCORD_ALLOW_BOTS", raising=False)
    monkeypatch.setenv("DISCORD_ALLOWED_USERS", "100200300")
    monkeypatch.setenv("DISCORD_TRUSTED_AGENT_USER_IDS", "1493785569602441337,1500372119744413817")

    source = _make_discord_bot_source(bot_id="1493785569602441337")
    assert runner._is_user_authorized(source) is True

    source = _make_discord_bot_source(bot_id="1500372119744413817")
    assert runner._is_user_authorized(source) is True


def test_discord_trusted_agent_bypass_is_exact_id_only(monkeypatch):
    """Only configured trusted-agent IDs get the special guidance bypass."""
    runner = _make_bare_runner()

    monkeypatch.delenv("DISCORD_ALLOW_BOTS", raising=False)
    monkeypatch.setenv("DISCORD_ALLOWED_USERS", "100200300")
    monkeypatch.setenv("DISCORD_TRUSTED_AGENT_USER_IDS", "1493785569602441337")
...
def test_discord_yomi_legacy_bypass_still_works(monkeypatch):
    """Legacy Yomi-only env remains supported for existing operators."""
    runner = _make_bare_runner()

    monkeypatch.delenv("DISCORD_ALLOW_BOTS", raising=False)
    monkeypatch.setenv("DISCORD_ALLOWED_USERS", "100200300")
    monkeypatch.setenv("DISCORD_YOMI_USER_IDS", "1493785569602441337")
