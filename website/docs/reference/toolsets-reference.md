...
| `x_search` | `x_search` | Search broad public X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool. Use `xurl` instead for raw/exact X API v2 search results, posting, DMs, timelines, follows, media, or other account-scoped API calls. Off by default; opt in via `hermes tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
